...
class VectorHopfield:
    ...
    ############################################
    def run_dynamics(self, initial_state, max_iter = 1000, stop_dq = 1-1e-10,verb=0, ORR=0., syncronous=False):
...
class TensorHopfield(VectorHopfield):
    ...
    def compute_local_field(self,i):
        raise Exception("Asyncronous version of tensor update TBA") 
        #return np.tensordot(self.state, self.J[:,i,:,i], axes=2)
    ...
    def get_local_magn(self):
        
        qq=np.zeros(self.N)
        
        for i in range(self.N):
            
            qq[i] = np.dot(self.examples[0,:,i], self.state[:,i])
...
class spectrumHopfield:
    ...
    def compute_eigenvectors_weights(self, neig):
        
        eigvecs_reshaped = np.reshape(self.eigenvectors, (self.N, self.d-1, (self.d-1)*self.N))
        
        return np.linalg.norm(eigvecs_reshaped[:,:,:neig], axis=1, keepdims=False)
    ...
    def change_eigenvector_representation(self): ## represents eigenvectors in the canonical base of R^{N\times d} (above they are computed in the space orthogonal to S)
